...
Describe 'Module Tests' -Tag Unit {
    Context "Module Tests" {
        ...
        It 'root module PsIni.psm1 should exist' {
            $PathToModule | Should -Exist
            $? | Should -BeTrue
        } #psm1Exists
        It 'manifest should contain PsIni.psm1' {
            $PathToManifest |
                Should -FileContentMatchExactly "PsIni.psm1"
        } #validPSM1
        ...
        It 'should have a valid project Uri' {
            $script:manifestEval.ProjectUri | Should -BeNullOrEmpty
        } #uri
    } #context_ModuleTests
